...
def edit_dwi_jsons(layout, subject, sessions):
    ...
    # Print the number of AP JSON files (for debugging purposes)
    print(f"Number of AP FMAP JSON files found: {len(AP_json)}")

    # Check if there are any FMAP JSON files to process
    if len(AP_json) > 1:
        assert(len(AP_json)==len(rel_dwi_paths)) ####check this out
        # Loop through all the AP FMAP JSON files
        for ap_json in AP_json:
            # print("ap_json:", ap_json)
            AP_json_path = os.path.join(ap_json.dirname, ap_json.filename)
            # print("AP_json_path:", AP_json_path)
            
            # Add the FMAP JSON path to the list of all JSON paths
            all_json_paths.append(AP_json_path)
            
            # Extract run information from the FMAP filename (e.g., sub-<subject>_ses-<session>_acq-dwi_dir-AP_run-01_epi.json)
            fmap_run = ap_json.filename.split('_run-')[1].split('_')[0]  # Extract the run number
            # print("fmap_run:", fmap_run)

            # Find the corresponding DWI file that matches the run from the FMAP
            matching_dwi_paths = []

            # Loop through all DWI files to find the one with the matching run
            for dwi_fn in rel_dwi_paths:
                # Check if the run number from the FMAP matches the DWI filename (e.g., run-01)
                if f"run-{fmap_run}" in dwi_fn:
                    # print(f"appending {dwi_fn} to matching_dwi_paths")
                    matching_dwi_paths.append(dwi_fn)

            if matching_dwi_paths:
                # Update the FMAP JSON to include the matching DWI file(s) in the IntendedFor field
                insert_edit_json(AP_json_path, 'IntendedFor', matching_dwi_paths)
                insert_edit_json(AP_json_path, 'PhaseEncodingDirection', 'j-')
            else:
                print(f"ERROR: No matching DWI run found for FieldMap {ap_json.filename}")
    else: 
    ...
    if PA_json:
        # Print the number of AP JSON files (for debugging purposes)
        print(f"Number of PA FMAP JSON files found: {len(PA_json)}")
        # Check if there are any FMAP JSON files to process
        if len(PA_json) > 1:
            # Loop through all the AP FMAP JSON files
            for pa_json in PA_json:
                # print("pa_json:", pa_json)
                PA_json_path = os.path.join(pa_json.dirname, pa_json.filename)
                # print("PA_json_path:", PA_json_path)
                
                # Add the FMAP JSON path to the list of all JSON paths
                all_json_paths.append(PA_json_path)
                
                # Extract run information from the FMAP filename (e.g., sub-<subject>_ses-<session>_acq-dwi_dir-AP_run-01_epi.json)
                fmap_run = pa_json.filename.split('_run-')[1].split('_')[0]  # Extract the run number
                # print("fmap_run:", fmap_run)
                ...
                if matching_dwi_paths:
                    # Update the FMAP JSON to include the matching DWI file(s) in the IntendedFor field
                    insert_edit_json(PA_json_path, 'IntendedFor', matching_dwi_paths)
                    insert_edit_json(PA_json_path, 'PhaseEncodingDirection', 'j-')
                else:
                    print(f"ERROR: No matching DWI run found for FieldMap {pa_json.filename}")
        else:
