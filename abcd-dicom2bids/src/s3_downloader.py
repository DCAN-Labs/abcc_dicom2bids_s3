    ...
    with open(log, 'w') as f:
        ...
        for sub in subject_list:
            ...
            for bids_year in year_list:
                ...
                sub_pass_QC_df = sub_ses_df[sub_ses_df['QC'] != 0.0] #changed this line back to be able to filter based on QC from fast track
                file_paths = []
                ### Logging information
...
def add_anat_paths(passed_QC_group, file_paths):
    ...
    T1_df = passed_QC_group[passed_QC_group['SeriesType'] == 'T1_NORM']
    if T1_df.empty:
        T1_df = passed_QC_group[passed_QC_group['SeriesType'] == 'T1']
        if T1_df.empty:
            has_t1 = 0 # No T1s. Invalid subject
        else:
            for file_path in T1_df['file_path']:
    ...
    T2_df = passed_QC_group[passed_QC_group['SeriesType'] == 'T2_NORM']
    if T2_df.empty:
        T2_df = passed_QC_group[passed_QC_group['SeriesType'] == 'T2']
        if T2_df.empty:
            has_t2 = 0 # No T1s. Invalid subject
        else:
            for file_path in T2_df['file_path']:
                file_paths += [file_path]
...
def add_func_paths(all_group,passed_QC_group, file_paths):
    ...
    if passed_QC_group['SeriesType'].isin(['rsfMRI', 'fMRI_MID_task', 'fMRI_nBack_task', 'fMRI_SST_task']).any():

        ## Pair SEFMs and only download if both pass QC
        #   Check first if just the FM exists
        FM_df = passed_QC_group[passed_QC_group['SeriesType'] == 'fMRI_FM']
        if FM_df.empty:
            ## Pair SEFMs first based on all fmaps available using the all_group def i.e. sub_ses_df before filtering for QC
            FM_AP_df = all_group[all_group['SeriesType'] == 'fMRI_FM_AP']
            FM_PA_df = all_group[all_group['SeriesType'] == 'fMRI_FM_PA']
            ...
            if FM_AP_df.empty:
                ...
                for i in range(0, upper_range):
                    if FM_AP_df.iloc[i]['QC'] != 0.0 and FM_PA_df.iloc[i]['QC'] != 0.0:
        ...
        if FM_df.empty:
            ...
            for file_path in FM_df['file_path']:
                file_paths += [file_path]
            has_sefm = FM_df.shape[0]


        ## List all rsfMRI scans that pass QC
        RS_df = passed_QC_group.loc[passed_QC_group['SeriesType'] == 'rsfMRI']
        if RS_df.empty:
            has_rsfmri = 0
        else:
            for file_path in RS_df['file_path']:
                file_paths += [file_path]
            has_rsfmri = RS_df.shape[0]

        ## List only download task if and only if there is a pair of scans for the task that passed QC
        MID_df = passed_QC_group.loc[passed_QC_group['SeriesType'] == 'fMRI_MID_task']

        if MID_df.empty:
            has_mid = 0
        else:
            for file_path in MID_df['file_path']:
                file_paths += [file_path]
            has_mid = MID_df.shape[0]
        SST_df = passed_QC_group.loc[passed_QC_group['SeriesType'] == 'fMRI_SST_task']
        if SST_df.empty:
            has_sst = 0
        else:
            for file_path in SST_df['file_path']:
                file_paths += [file_path]
            has_sst = SST_df.shape[0]
        nBack_df = passed_QC_group.loc[passed_QC_group['SeriesType'] == 'fMRI_nBack_task']
        if nBack_df.empty:
            has_nback = 0
        else:
            for file_path in nBack_df['file_path']:
                file_paths += [file_path]
...
def add_dwi_paths(all_group, passed_QC_group, file_paths):
    DTI_df = passed_QC_group.loc[passed_QC_group['SeriesType'] == 'dMRI']
    if DTI_df.shape[0] >= 1:
        # If a DTI exists then download all passing DTI fieldmaps
        DTI_FM_df = passed_QC_group.loc[passed_QC_group['SeriesType'] == 'dMRI_FM']
        # If not present, next search and sort AP/PA fmaps
        if DTI_FM_df.empty:
            DTI_FM_AP_df = all_group[all_group['SeriesType'] == 'dMRI_FM_AP']
            DTI_FM_PA_df = all_group[all_group['SeriesType'] == 'dMRI_FM_PA']
            ...
            if DTI_FM_AP_df.empty:
                ...
                for i in range(0, upper_range):
                    if DTI_FM_AP_df.iloc[i]['QC'] != 0.0 and DTI_FM_PA_df.iloc[i]['QC'] != 0.0:
                        DTI_FM_df = DTI_FM_df.append(DTI_FM_AP_df.iloc[i])
                        DTI_FM_df = DTI_FM_df.append(DTI_FM_PA_df.iloc[i])
        if not DTI_FM_df.empty:
            for file_path in DTI_df['file_path']:
                file_paths += [file_path]
            for file_path in DTI_FM_df['file_path']:
                file_paths += [file_path]
